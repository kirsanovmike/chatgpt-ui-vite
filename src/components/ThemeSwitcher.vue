  ...
  radios.forEach((radio) => {
    radio.addEventListener('change', () => {
      if (radio.checked) {
        ...
        if (previousValue) previous.value = previousValue
      }
    })
  })
}
...
watch(mode, (value) => {
  if (value && value !== selected.value) selected.value = value as ThemeName
})

onMounted(() => {
  if (switcherRef.value) {
    const preset = switcherRef.value.querySelector<HTMLInputElement>(`input[value="${selected.value}"]`)
