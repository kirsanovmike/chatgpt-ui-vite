    ...
    <v-container>
      <v-row>
        <v-col sm="9" offset-sm="1" md="8" offset-md="2">
          <v-card class="mt-20vh" elevation="0">
            <div class="text-center">
              ...
              <div v-else>
                ...
                <p class="mt-5">
                  We've sent a verification email to <strong>{{ user?.email ?? 'your email' }}</strong>.<br>
...
const resendEmail = async () => {
  errorMsg.value = null
  sending.value = true
  const { error } = await useFetch('/api/account/registration/resend-email/', {
    method: 'POST'
  })
  if (error.value) {
